...
for report in reports:
    ...
    url = "/".join(get_urls[:-2]).replace("./reports/","").strip()
    with open(report, 'r') as fh:
        data = json.load(fh)

    for d in data['incomplete']:
        for node in d['nodes']:
            errors_fh.append([url, testbrowse,d['description'].strip(), d['id'], node['failureSummary'].strip(), node['html'], node['impact'], "incomplete" ])

    for d in data['violations']:
        for node in d['nodes']:
            errors_fh.append([url, testbrowse,d['description'].strip(), d['id'], node['failureSummary'].strip(), node['html'], node['impact'], "violations" ])
